...
class Particle {
  constructor(options, firstFrame) {
    ...
    this.radius =
      Math.floor(Math.random() * (options.size.max - options.size.min + 1)) +
      options.size.min;
    ...
    var speed =
      Math.random() * (options.speed.max - options.speed.min) +
      options.speed.min;
    ...
    if (options.style.fill.color[0] == "#") {
      // Hex color code
      this.fill = hexToRGBA(
        options.style.fill.color,
        options.style.fill.opacity
      );
    } else {
...
function genParticles(options, callback) {
  ...
  options.canvas.element.style.cssText =
  ...
  const timerID = setInterval(function () {
    ...
    // redraw particles
    options.canvas.ctx.clearRect(
      0,
      0,
      options.canvas.element.width,
      options.canvas.element.height
    );
